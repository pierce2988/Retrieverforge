...
const RFGauge = (() => {
  ...
  function bigDial({ score = 94, size = 220, id = 'big', color = '#F97316' } = {}) {
...
<svg viewBox="0 0 ${size} ${size}" width="${size}" height="${size}" class="rf-dial">
  <defs>
    <linearGradient id="dialGrad-${id}" x1="0" y1="1" x2="1" y2="0">
      <stop offset="0" stop-color="#C2580E"/>
  ...
  <text x="${cx}" y="${cy-4}" text-anchor="middle" font-family="JetBrains Mono, monospace" font-size="${size*0.21}" font-weight="700" fill="#F5F2EA">${score}</text>
  ...
  function miniDial({ score = 90, label = 'Health', id = 'mini', light = false } = {}) {
    ...
    const circumference = 2 * Math.PI * r * (270/360);
    const dashOffset = circumference * (1 - score/100);
    const trackColor = light ? 'rgba(13,15,17,0.1)' : 'rgba(245,242,234,0.08)';
    const textColor = light ? '#0D0F11' : '#F5F2EA';
...
<div class="rf-mini-dial">
  <svg viewBox="0 0 ${size} ${size}" width="${size}" height="${size}">
    ...
    <path d="${trackPath(cx,cy,r)}" fill="none" stroke="#F97316" stroke-width="7" stroke-linecap="round"
          stroke-dasharray="${circumference}" stroke-dashoffset="${dashOffset}"/>
